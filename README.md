...
將來源各異（社群散文、影片逐字稿、本地 `.docx`、`.pdf` 文件、poe.ninja 天梯鏡像）的散亂攻略，毫秒級提取並標準化轉換為結構嚴謹、機制透徹、**國際服官方繁體中文規範（對齊 poe2db.tw）**、且包含「實機技能表格矩陣」與「相對路徑高清截圖」的本地自包含指南。

---

## ✨ 核心特性與設計原則 (Zero-Tolerance Rules)

1. **官方繁中單一可信源（SSOT: poe2db.tw）**：
   - 徹底杜絕機械簡轉繁造成的詞彙割裂（如：國服「宝石术士/雇佣兵/榴弹/极速攻击」一律自動精確對齊官方繁中正名「**古靈軍團/傭兵/擲彈/疾速攻擊**」）。
2. **截圖即第一數據源（Screenshots as Primary SSOT）**：
   - PoE2 機制精密，作者攻略常有「正文不寫連線，全靠遊戲截圖」的習慣。本工具強制對齊圖文時序流，像素級精確轉錄實機 Tooltip，**嚴禁腦補或使用「【輔助 1】」等佔位符**。
3. **實機技能表格矩陣（Skills Matrix）**：
   - 主動技能、等級/品質、1~5 顆輔助寶石、武器套組綁定與機制注意事項一目了然。
4. **探索彩蛋、數值檔位與邪道技巧強制提取**：
   - 主動收錄跑圖領取免費天賦點、首飾塗油卡額外技能槽、水晶化免疫藍寶石規則、怒火倍數臨界點、低成本淘寶技巧。
5. **本地自包含交付，自由選擇存儲路徑**：
   - **完全去耦第三方服務**：無需配置繁瑣的 Webhook 或 API Token，絕不報錯。
   - **隨心存放**：生成的文件可直接存入你的 **Obsidian 知識庫**、本地自定義文件夾或雲盤同步盤，Markdown 配合本地 `images/` 相對路徑開箱即看。
   - **可選 Notion 打包**：若需要導入 Notion，提供一鍵平鋪 ZIP 打包腳本（消除套娃，繞過 5MB 上傳限制）。
6. **零外部依賴 (Zero External Dependencies)**：
